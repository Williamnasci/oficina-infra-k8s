...
> **Conta AWS Academy Learner Lab (não mais conta pessoal):** este projeto roda numa conta de sandbox do AWS Academy — orçamento fixo de USD 50 para todo o curso, sessão de lab de ~4h (renovável), e a própria plataforma **termina a instância EC2 automaticamente ao fim de cada sessão** (não precisamos mais destruí-la manualmente entre sessões de trabalho). O `LabRole` da conta também não permite criar usuário/role IAM permanente (`iam:CreateUser`/`iam:CreateRole`/`iam:PutRolePolicy` são negados) — por isso o CI/CD usa credenciais de sessão temporárias (ver seção CI/CD abaixo), o `apply` deixou de ser automático no merge, e a EC2 usa o instance profile compartilhado `LabInstanceProfile` pré-provisionado pela plataforma em vez de uma role própria escopada (ver comentário em `iam.tf`).
